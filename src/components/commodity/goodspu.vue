<template>
  <div id="spu">
    ...
    <el-table :data="goodspus" stripe style="width: 100%">
      ...
      <el-table-column prop="brand" label="生产商" width="100">
      </el-table-column>
      <el-table-column prop="sort.sortName" label="商品分类" width="100">
      </el-table-column>
      <el-table-column prop="statusStr" label="上架状态" width="100">
      </el-table-column>
      <el-table-column prop="user.username" label="创建人" width="100">
      </el-table-column>
      <el-table-column prop="createTime" label="创建时间" width="100">
      </el-table-column>
      <el-table-column prop="user.userRenewName" label="修改人" width="100">
      </el-table-column>
...
export default {
  data() {
    return {
      ...
      dialogFormVisible : false,
       rules: {
        goodName: [{ required: true, message: "请输入商品名", trigger: "blur" }],
        brand: [{ required: true, message: "请输入生厂商", trigger: "blur" }],
        ['sort.sortName']: [{ required: true, message: "请输入分类", trigger: "blur" }],
  ...
  methods: {
    ...
    add() {
      ...
      this.axios.post("http://localhost:8088/goodsku/insertGoodSpu",this.goodspu).then((response) => {
          if (response.status === 200) {
            this.$alert("添加成功");
