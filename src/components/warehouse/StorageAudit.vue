      ...
      <table id="table1" cellspacing="0">
        ...
        <tr>
          <td colspan="5" rowspan="2">
            <textarea
              v-model="gridData.inventoryDetails"
...
export default {
  methods: {
    ...
    // 通过
    sub() {
      if (this.gridData.status != 5) {
        this.gridData.status = 200;
      }
      this.gridData.token = sessionStorage.getItem('token');
      this.axios
        .put('http://localhost:8088/warehousing/inventoryAudit', this.gridData)
        .then((res) => {
          let data = res.data;
          if (data.code === 200) {
